...
def get_layout(pathname, search):
    ...
    try:
        ...
        features = None
    # # url = f"http://fit.lc.snad.space/api/v1/models"
            ...
            html.Div(
                [
                    html.Div(
                        [
                            html.Div([
                                dcc.Dropdown(model_fit.models(), id='models-fit-dd'),
...
def update_output(
    ...
    if name_model:
        ...
        params = model_fit.fit(df, name_model)
...
def set_figure(
    ...
    if name_model and fit_params:
        ...
        df_fit = model_fit.get_curve(extract_values(json.loads(str(fit_params).replace("'", '"').replace('*', ''))),
                                     name_model, band_ref, bright, ['ztf' + str(band[1:]) for band in df["filter"].unique()],
